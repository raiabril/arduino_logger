...
def openSerial(strPort, baudRate):
...
def retrieveSamples(ser, user_input, delay):
    time = []
    data1 = []
    data2 = []
    command = str(int(user_input)) + '/' + str(int(delay)) + '/'
    ser.write(command)
    for i in range(1, int(user_input)+1):
        line = ser.readline()
        print line
        line = line.split()
        timeNum = float(line[0])
        data1Num = float(line[1])
        data2Num = float(line[2])
        time.append(timeNum)
        data1.append(data1Num)
        data2.append(data2Num)
...
class CanvasPanel(wx.Panel):
    ...
    def draw(self, data):
        self.axes.plot(data[0], data[1],'b.', label = 'data1')
        self.axes.plot(data[0], data[2], 'k.', label = 'data2')
        order = 6
        fs = 20.0 # Sample rate in Hz
        cutoff = 3.667  
        data1_filtered = butter_lowpass_filter(data[1], cutoff, fs, order)
        data2_filtered = butter_lowpass_filter(data[2], cutoff, fs, order)
        self.axes.plot(data[0], data1_filtered, 'b-')
        self.axes.plot(data[0], data2_filtered, 'k-')
        self.axes.legend()
...
class MyFrame1 ( wx.Frame ):
	...
	def __init__( self, parent ):
	        box = wx.SingleChoiceDialog(None, 'Available ports', 'Select port!', serial_ports() )
                if box.ShowModal() == wx.ID_OK:
                    self.port = box.GetStringSelection()
		...
		# Events.
                ...
                self.m_dirPicker2.Bind( wx.EVT_DIRPICKER_CHANGED, self.updateDir )
	...
	def __del__( self ):
        ...
        def OnGo(self,e):
            samples = self.m_textCtrl1.GetValue()
            frequency = self.m_textCtrl2.GetValue()
            baudRate = 9600
            ser = openSerial(self.port, baudRate)
            
            delay = 1000/float(frequency)
            data = retrieveSamples(ser, samples, delay)
            closeSerial(ser)
            self.panel_plot.axes.cla()
            self.panel_plot.draw(data)
            self.panel_plot.canvas.draw()
            self.panel_plot.canvas.Refresh()
            self.Show()
            
        def updateDir(self, e):
            newpath = self.m_dirPicker2.GetPath()
            os.chdir(newpath)
            print newpath
